...
// "users" — نفس الجدول اللي يخدمو بيه login.html و cartefidelite.html
// (ماشي جدول "loyalty" منفصل) باش نقاط الوفاء تبقى متوافقة بين الموقعين
const usersRef = collection(db, "users");
const wheelResultsRef = collection(db, "prizes");
// "locations" — نفس الأسماء والحقول اللي يكتبهم index.html متاع الموقع
// الأساسي (latitude/longitude/timestamp/device/browser)
...
window.openAddProductModal = function() {
  ...
  document.getElementById("productCurrency").value = "د";
...
window.editProduct = function(id) {
  ...
  document.getElementById("productCurrency").value = product.currency || "د";
...
window.saveProduct = async function() {
  ...
  const currency = document.getElementById("productCurrency").value.trim() || "د";
  ...
  list.innerHTML = filtered.map(p => {
    ...
    <div class="product-card ${p.available === false ? 'unavailable' : ''}">
      ...
      <div class="product-body">
        ...
        <div class="product-price">${p.price} ${p.currency || 'د'}</div>
        ...
        ${variants.map(v => `
          <div class="variant-item">
            ...
            <div class="variant-price">
              ${v.price > 0 ? '+' : ''}${v.price || 0} ${product?.currency || 'د'}
            </div>
...
// =========================
// LOYALTY (نقاط الوفاء) — جدول "users" نفسو المستعمل فـ
// login.html / cartefidelite.html متاع الموقع الأساسي
// =========================
function loadUsers() {
  const body = document.getElementById("loyaltyBody");
  if (!body) return;
  onSnapshot(usersRef, (snap) => {
    let totalUsers = 0, totalPoints = 0, rewardUsers = 0;
    const rows = [];
    snap.forEach(d => {
      const data = d.data();
      const uid = d.id;
      ...
      rows.push(`
        <tr>
          ...
          <td><span class="badge badge-gold">⭐ ${points}</span></td>
          <td>${points >= 10 ? '🎉 ✅' : '❌'}</td>
          <td>
            <button class="btn btn-sm btn-primary" onclick="addPoint('${uid}')">➕</button>
            <button class="btn btn-sm btn-secondary" onclick="removePoint('${uid}')">➖</button>
            <button class="btn btn-sm btn-outline" onclick="resetPoints('${uid}')">↺</button>
            <button class="btn btn-sm btn-gold" onclick="rewardUser('${uid}')" ${points < 10 ? 'disabled' : ''}>🎁</button>
          </td>
        </tr>
      `);
    });
    updateStat("statCustomers", totalUsers);
    updateStat("statPoints", totalPoints);
    updateStat("statFreeMeals", rewardUsers);
    body.innerHTML = rows.length ? rows.join("") : '<tr><td colspan="4" class="loading-cell">لا يوجد زبائن</td></tr>';
  }, (error) => {
    body.innerHTML = `<tr><td colspan="4" class="loading-cell">❌ خطأ: ${error.message}</td></tr>`;
  });
}

window.addPoint = async function(uid) {
  try {
    await updateDoc(doc(db, "users", uid), { points: increment(1) });
    showToast("✅ +1 نقطة");
  } catch (e) {
...
window.removePoint = async function(uid) {
  try {
    ...
    if (current <= 0) {
      showToast("⚠️ هذا الزبون عنده 0 نقاط", "warning");
      return;
    }
...
window.resetPoints = async function(uid) {
  if (!confirm("⚠️ متأكد تحب ترجع النقاط للصفر؟")) return;
  try {
    await updateDoc(doc(db, "users", uid), { points: 0, reward: false });
...
window.rewardUser = async function(uid) {
  if (!confirm("🎁 متأكد تحب تمنح وجبة مجانية لهذا الزبون؟")) return;
  try {
    const userRef = doc(db, "users", uid);
    const snap = await getDoc(userRef);
    if (!snap.exists()) return;
    ...
    if (points < 10) {
      showToast("⚠️ هذا الزبون ما عندهش 10 نقاط", "warning");
      return;
    }
    await updateDoc(userRef, { points: increment(-10), reward: true });
    showToast("🎉 تم منح الوجبة المجانية! -10 نقاط");
  } catch (e) {
    showToast("❌ خطأ: " + e.message, "error");
  }
};
...
function loadWheelResults() {
  ...
  onSnapshot(q, (snap) => {
    ...
    snap.forEach(d => {
      ...
      rows.push(`
        <tr>
          <td>${res.time || "غير معروف"}</td>
          <td style="color:gold;font-weight:bold;">${res.prize || "..."}</td>
        </tr>
      `);
    });
    body.innerHTML = rows.length ? rows.join("") : '<tr><td colspan="2" class="loading-cell">📭 لا توجد نتائج</td></tr>';
  }, () => {
    body.innerHTML = '<tr><td colspan="2" class="loading-cell">📭 لا توجد نتائج بعد</td></tr>';
  });
}
...
window.saveWheelSchedule = async function() {
  ...
  try {
    await setDoc(wheelScheduleRef, { start, end, updatedAt: Date.now() }, { merge: true });
...
// =========================
// EVENTS — نفس حقول evenment.html (title/description/date/time/image)
// =========================
function loadEvents() {
    ...
    snap.forEach(d => {
      ...
      div.innerHTML = `
        ...
        <div class="event-body">
          ...
          <small>📅 ${data.date || ''} ${data.time ? '⏰ ' + data.time : ''}</small>
        </div>
...
window.publishEvent = async function() {
  ...
  if (!title || !date) { showToast("أدخل عنوان وتاريخ الإيفينمون", "error"); return; }
  try {
    await addDoc(eventsRef, { title, description, date, time, image, createdAt: Date.now() });
...
// =========================
// LOCATIONS — نفس حقول index.html متاع الموقع الأساسي
// (latitude/longitude/timestamp/device/browser/googleMapsUrl)
// =========================
function loadLocations() {
  ...
  onSnapshot(q, (snap) => {
    let all = [];
    let android = 0, ios = 0, todayCount = 0;
    const today = new Date(); today.setHours(0, 0, 0, 0);
    snap.forEach(d => {
      const data = d.data();
      const ts = data.timestamp && data.timestamp.toDate ? data.timestamp.toDate() : null;
      all.push({ id: d.id, ...data, _date: ts });
      const dev = (data.device || "").toLowerCase();
      if (dev.includes("mobile") || dev.includes("android")) android++;
      else if (dev.includes("tablette") || dev.includes("iphone") || dev.includes("ipad")) ios++;
    ...
    body.innerHTML = '<tr><td colspan="5" class="loading-cell">📭 لا توجد بيانات مواقع بعد</td></tr>';
  });
}
...
function renderLocations(all) {
  ...
  if (currentLocationFilter === "today") {
    const today = new Date(); today.setHours(0, 0, 0, 0);
    filtered = all.filter(l => l._date && l._date >= today);
  } else if (currentLocationFilter === "7days") {
    filtered = all.filter(l => l._date && (now - l._date.getTime()) <= 7 * day);
  } else if (currentLocationFilter === "30days") {
    filtered = all.filter(l => l._date && (now - l._date.getTime()) <= 30 * day);
  }
  if (filtered.length === 0) {
    body.innerHTML = '<tr><td colspan="5" class="loading-cell">لا توجد بيانات</td></tr>';
    return;
  }
  body.innerHTML = filtered.map(l => {
    const d = l._date || new Date();
    const mapsUrl = l.googleMapsUrl || (l.latitude && l.longitude ? `https://www.google.com/maps?q=${l.latitude},${l.longitude}` : "#");
    return `
      <tr>
...
window.sendNotification = async function() {
  ...
  try {
    ...
    showToast("تم تسجيل الإشعار — دورّ لـ Firebase Console باش ترسلو فعلياً", "success");
  } catch (err) {
    showToast("خطأ: " + err.message, "error");
  }
};
...
console.log("✅ Admin panel (standalone) loaded — connected to shared Firebase project");
